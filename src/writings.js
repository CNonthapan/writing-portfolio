/*
export const writings = {
  essays: [
    {
      id: "your-essay-slug",
      title: "Your Essay Title",
      content: `
Your essay content in **Markdown** goes here.

You can include paragraphs, quotes, and line breaks.

> Example quote or excerpt.

More text here.
      `,
      pdf: "/assets/your-essay.pdf", // optional — remove if not needed
    },
  ],
  stories: [
    {
      id: "your-story-slug",
      title: "Your Story Title",
      content: `
Story text here, written in Markdown.
      `,
    },
  ],
  poetry: [
    {
      id: "your-poem-slug",
      title: "Your Poem Title",
      content: `
Line one  
Line two  
Line three
      `,
    },
  ],
};
*/

export const writings = {
  essays: [
    {
      id: "honors-english",
      title: "Honors English Prompt Response",
      content: `

>Wrote this on the first day of school after summer break of last year for Honors English. Isn't particularly notable but I don't have many essays. Leaving this here to fill the gaps

I have been awake for roughly thirty consecutive hours, trying to force my circadian rhythm back into something normal. The sky was still wearing its nightshirt when I left home at 0600—an hour I had not seen voluntarily in months. The world felt untouched. Empty roads, blinking traffic lights with no one to command.

I stopped at a Foodland near school, its fluorescent lights a harsh contrast to the soft, tentative morning. Inside, time moved differently. A few old men, bent with memory and ritual, sat scattered across the diner, sipping black coffee without sugar, without hurry, without distraction. I joined them in their quiet ritual and ordered a full English and coffee.

The breakfast arrived, and I couldn’t help but smirk. It was not “full,” nor was it particularly “English.” No baked beans. No mushrooms. No blood sausage. A couple of eggs, toast, a sausage, and a pitiful slice of tomato that felt more like an apology than part of a meal. I don’t even like baked beans or tomatoes. But what stood out to me was how a misleading menu item had simultaneously bothered and amused me so much.  How often do we take menu items—or people, or days—for what they promise rather than what they are?

This misnamed breakfast, served in a half-empty diner to the sound of clinking cups and murmured conversations, gave me something rare: presence. For the first time in a long time, I noticed. The flavor of coffee. The rhythm of my own breathing. The warmth of the plate against my fingertips. There was no rush. No obligation.

Most days, I wakeup at 0700 . I skip breakfast, shove myself into a routine like a garment I’ve outgrown, and leave the house already ten minutes behind. I run through each day like I’m trying to catch up with something I can’t name. And in the rush, I forget to look. Forget to feel. Forget to _be_.

But not today. Today I remembered that the little, inconsequential things—so often overlooked—are often what stitch our lives together. The smell of morning rain. The way the light slants across a desk. A meal eaten alone, without urgency.

After that brief pause in time, I returned to school, returned to the day, and resumed pushing the boulder. We all have one. Mine is forged of expectations, ambitions, assignments, roles I play for others, and ones I haven’t yet learned to play for myself. Every morning, I shoulder it anew. I push it up the hill, knowing full well it will roll back by evening.

For a short while this morning, I stopped asking _why_. Why this boulder? Why this hill? Why this absurd, endless labor? I simply accepted the task. And in that acceptance, I understood something that many philosophers have stumbled upon.

I thought of Sisyphus—forever condemned to push his stone up the slope, again and again, and as expected, he was happy.

We are not so different him. We wake. We push. We rest. We repeat. But in that loop, there are moments—if we’re lucky—where the sun catches our face just right, the coffee tastes just bitter enough, and the silence tells us we’re alive.

And maybe that’s enough.
      `,

    },
    {
      id: "tin-mine",
      title: "บทเรียนจาก เรื่องสั้นชุดเหมืองแร่",
      content: `
>กินอย่าอาย ตายอย่ากลัว ยากช่างหัว ตายปลด

ผมโตมาในโรงเรียนเอกชนตั้งแต่จำความได้ ห้องเรียนของผมมีกระจกใส แอร์เย็น พื้นสะอาด ไม่มีเศษดินติดรองเท้า แต่โลกของ อาจินต์ ปัญจพรรค์ ใน “เหมืองแร่” กลับเต็มไปด้วยเหงื่อ ขี้โคลน และเสียงหัวเราะของคนงานที่ไม่รู้จักคำว่า “หลักสูตรแกนกลาง” สิ่งที่เขาเรียนรู้ไม่ได้มาจากตำรา แต่มาจากการถูกโยนลงไปในเหมืองที่สอนบทเรียนชีวิตที่ตรงกว่า ครูคือความลำบาก หนังสือคือแผลที่มือ

เรื่องสั้นชุดเมืองแร่นั้นมีความสำคัญต่อผมมาก เพราะมันทำให้ผม ลูกหลานคนใต้ที่เติบโตห่างจากปักษ์ใต้ ให้ได้เข้าใจรากเหง้าของตนเอง ผมเกิดที่กรุงเทพฯ และใช้ชีวิตที่นนทบุรีตั้งแต่เด็ก ซึ่งบ้านผมนั้นอยู่ห่างจากป้ายที่เขียนว่า “ยินดีต้อนรับสู่จังหวัดนนทบุรี” แค่ไม่กี่กิโล จึงทำให้ชีวิตประจำวันผมนั้นไม่ต่างกับคนกรุงมากนัก เมื่อมีคนถามผมว่าผมเป็นคนจากที่ไหน ผมก็มักจะตอบไปว่าบ้านผมเป็นคนระนอง แล้วก็แหลงใต้ให้เขาฟังซักสองสามคำ แต่แท้จริงแล้วผมก็ไม่เคยไปอยู่ปักษ์ใต้เกินสัปดาห์ 

ความดึงดูดที่ผมมีต่อปักษ์ใต้นั้นอาจมีที่มาจาก ตอนที่พ่อผมพาไประนองตอนยังเด็กอยู่ เส้นทางอันคดเคี้ยวแสนยากลำบาก ทำให้ผมไม่อยากนึกเลยว่าการเดินทางลงใต้นั้นจะยากแค่ไหนในยุคนั้น ทุกครั้งที่ผมอ่าน เหมืองแร่ ผมจะนึกถึงฝนใต้ ฝนที่ตกหนักและยาวนาน ราวกับไม่มีวันหยุด ฝนที่ชะล้างทุกสิ่งลงดิน ฝนที่ทำให้ภูเขาเปียกจนละลาย ฝนที่ตกจนทำให้ผมในตอนนั้นนึกขึ้นว่า โลกใบนี้จะขาดแคลนน้ำได้อย่างไรถ้าฝนมันตกขนาดนี้

ผมคิดว่าฝนที่ผมเจอคงเป็นฝนเดียวกันที่สอนอาจินต์ มันตกทุกวันจนคนเลิกบ่น มันสอนให้คนอดทนและไม่หวังพึ่งฟ้า มันสอนว่าอำนาจไม่ได้อยู่บนฟ้าแต่อยู่ที่สองมือ มันคือฝนที่ซึมเข้าไปในถ้อยคำของเขา ทำให้ เหมืองแร่ ชุ่มไปด้วยชีวิตจริง ไม่ใช่ความฝัน ไม่ใช่ละคร จนผมเองยังได้กลิ่นดินนั้นทุกครั้งที่อ่าน

อาจินต์ ปัญจพรรค์ ไม่ได้เริ่มต้นชีวิตในเหมือง เขาเริ่มต้นในกรุงเทพฯ ใต้หลังคาแห่ง จุฬาลงกรณ์มหาวิทยาลัย สถานที่ที่เป็นสัญลักษณ์ของความรู้ ความสำเร็จ และความหวังของชนชั้นกลางไทยในยุคหลังสงครามโลก แต่โชคชะตากลับหักเลี้ยว เขาต้องออกจากคณะวิศวกรรมศาสตร์ในปีที่สอง ชีวิตที่เคยมีกรอบของการศึกษาจึงพังลงในพริบตา เขาไม่ได้กลายเป็น วิศวกร อย่างที่ใครคาดหวัง

ผมลองจินตนาการว่าถ้าเขาเรียนจบ ชีวิตของอาจินต์คงกลายเป็นอีกหนึ่งวิศวกรธรรมดาในกรุงเทพฯ ที่ไม่เคยเข้าใจค่าของคำว่า “อยู่รอด” แต่เพราะความล้มเหลวนั้น เขาจึงได้เห็นโลกจริง เพราะความล้มเหลวนั้นเอง เพราะวันที่เขาต้องเดินทางลงใต้โดยไม่มีแม้แต่เส้นทางชัดเจน เขาจึงได้พบโลกจริง โลกที่คนเรียนไม่สูงแต่มีปัญญา โลกที่สอนให้เข้าใจว่าศักดิ์ศรี ไม่ได้อยู่ในใบปริญญา แต่อยู่ในการทำมาหากินอย่างสุจริต

และบางที ถ้าเขาไม่ล้มในวันนั้น เขาอาจไม่มีวันลุกขึ้นมาเป็น “อาจินต์ ปัญจพรรค์” ที่เรารู้จักในวันนี้ เขาคงไม่กลายเป็นนักเขียนที่มองเห็นความงดงามในความหยาบกระด้างของชีวิต ไม่ได้เขียน “เหมืองแร่” ที่กลั่นออกมาจากประสบการณ์แท้จริง และที่สำคัญ เขาคงไม่เคยได้เป็น “ศิลปินแห่งชาติ” ผู้ใช้ถ้อยคำธรรมดาแต่เฉียบแหลมจนถึงผมเอง เด็กนักเรียนจากครอบครัวที่มีฐาะ ยังได้อ่านงานเขียนที่มีอายุมากกว่าผมถึงสี่ห้าเท่า

คำว่า “กินอย่าอาย ตายอย่ากลัว ยากช่างหัว ตายปลด” เป็นเหมือนหัวใจของเรื่อง และเป็นประโยคที่สะเทือนผมในฐานะคนที่ถูกเลี้ยงมาในโลกที่ ความวัตถุนิยมเป็นเรื่องสำคัญ แต่คนงานในเหมืองของอาจินต์กลับเรียนรู้ที่จะไม่อายต่อการมีชีวิต จะกิน จะเหนื่อย จะลำบาก ก็ไม่ต้องซ่อน เพราะศักดิ์ศรีแท้คือการอยู่รอดอย่างไม่โกหกตัวเอง

>มหาวิทยาลัยสอนความรักชาติ แต่เหมืองแร่สอนความรักชีวิต

"อาย" "กลัว" "ยาก" "ปลด" สี่คำนี้อาจเรียบง่าย แต่รวมกันแล้วกลายเป็นปรัชญาชีวิตที่ลึกกว่าที่จะหาได้ในหนังสือเรียน การไม่อายต่อการทำมาหากิน ไม่กลัวต่อความตาย ไม่หวั่นต่อความยาก คือหนทางของคนที่เข้าใจคุณค่าของการอยู่รอด

ในระบบการศึกษา เราไม่ค่อยได้ถูกฝึกให้เข้าใจชีวิต เรารู้สูตร แต่ไม่รู้สึก เราท่องจำ แต่ไม่ถามว่าทำไม แต่เหมืองแร่ของอาจินต์กลับเป็นโรงเรียนที่ทุกเช้าเริ่มด้วยเสียงเรือขุดและจบลงที่วงเหล้า เป็นโรงเรียนที่สอนให้กลัวน้อยลง อายให้น้อยลง และเข้าใจคำว่า “อยู่” ให้มากขึ้น รอยแผลบนมือของอาจินต์คือประกาศนียบัตร จากสถาบันที่ไม่มีชื่อในทะเบียนใด แต่มีค่าเหนือปริญญาใดๆ

>เราไม่มีเวลาชมจันทร์บนชายคา แต่ไปงมจันทร์ในก้นแก้วกัน

เมื่อผมมองย้อนกลับไปในชีวิตที่ไม่เคยขาดแคลน ผมรู้ว่าผมไม่เคยรู้จริงว่า “ยาก” หมายถึงอะไร “เหมืองแร่” จึงเป็นเหมือนครูอีกคน ที่บอกผมอย่างเรียบง่ายว่า ชีวิตไม่ได้มีแต่แสงไฟในห้องเรียน มันยังมีเหงื่อและฝุ่น ที่บางครั้งส่องประกายกว่ารางวัลใดๆ

ผมก็ตลกกับความย้อนแย้งที่ตัวเองมาพร่ำเพ้อเรื่อง ประสบการณ์สำคัญกว่าตำรา ทั้งที่สิ่งเหล่านี้ผมดันได้มาจากหนังสือเล่มหนึ่ง ผมที่เติบโตในความสบาย กลับมาซึ้งใจกับคำของคนที่เรียนไม่จบ ผมที่ไม่เคยจับจอบจับเสียมเสียด้วยซ้ำ กลับรู้สึกเหมือนเข้าใจหยาดเหงื่อและงานหนักบนเรือขุดได้จากงานเขียนของเขา

สุดท้ายแล้วเหมืองแร่ คือบทพิสูจน์ว่าความล้มเหลวไม่ใช่รอยด่าง แต่คือทางเปิดของชีวิตที่ทำให้มองเห็นเนื้อแท้ของชีวิตคนเรา การศึกษาที่แท้ ไม่ได้วัดจากเกรดหรือชื่อเสียงโรงเรียน แต่วัดจากความกล้าที่จะทำมาหากินโดยไม่อาย กล้าที่จะทนอยู่กับความลำบาก กล้าที่จะล้มโดยไม่กลัว และกล้าที่จะอยู่โดยไม่หนีความจริง

>อดีตคือความฝัน ปัจจุบันต้องอด อนาคตต้องตาย
      `,

    },
  ],
  stories: [
    {
      id: "a-choice-of-two",
      title: "A Choice of Two",
      content: `
  The air curtain blows on me as I step inside, cold air hitting my skin. The air
carries a scent of dust and something metallic, like an old film reel that’s been
sitting too long in an attic. Display cases stretched from wall to wall, stacked with
countless cameras and lenses in neat, disciplined rows—Old school SLRs, DSLRs,
mirrorless, compacts—you name it.

  The man behind the counter looks up from a battered photography magazine.
He gives me a nod, the kind reserved for people who don’t really know what they
want.

  I used to come here with him.

  We’d linger by the display cases, our breath fogging the glass as we argued
about what makes a photographer great. He swore by a minimalist entry-level
camera, while I adamantly stuck to a more robust, professional one.

  He never cared about specs. To him, it’s just a box that lets you steal
moments, and the rest was down to the hands that held it. He never cared if it was
an entry-level compact from a decade ago or the latest state of the art mirrorless.
He had this way of making a photo feel more than pixels, like it carried to air of
that moment, the way the world was before it moved on.

  I obsessed over the details. Aperture, ISO range, sensor sizes, max shutter
speeds. I spent hours watching reviews, scrolling through endless comparison,
looking at technical stats like it was the key to unlock some hidden truth. He used
to make fun of me for having “Gear Acquisition Syndrome.” In a sense, he was right.
I buy a new lens, then I buy the next in a vain attempt to achieve the next level of
photography.

  “You don’t trust your skills,” he told me once, looking through the specs sheet
of a new camera that I showed him. “ You think if you bought the perfect camera,
you’ll finally take the perfect shot. But it’s never the camera, is it?”

  I hated that he was right.

  Now, he’s six thousand miles away, in a city where the rain smells different,
where the light falls differently, where he points his lens at strangers I’ll never
meet. He took nothing fancy with him, just a small mirrorless his dad bought for
him years ago and a simple nifty fifty, something that fit into a pocket, something
that won’t get in the way.

  By the window, a mirrorless, the flagship model, gleamed under the display
light. I pick it up, turning it over in my hands, sleek, impossibly light for its size,
nothing like the bulk of a DSLR. It feels effortless.

  “That’s one of our most cutting-edge professional mirrorless,” the man behind
the counter says, “45 megapixel full-frame sensor, shoots sharp, great in low light,
lightning fast autofocus, you won’t ever miss a shot.”

  “That’s the problem,” I mutter before I can stop myself.

  The old me—the me before him—would’ve loved it. But I’ve started noticing
the way imperfection tells more story than something flawlessly composed. The way
the weight of a camera slows you down, makes you think before you shoot.

  He hated DSLRs. “Too much in the way,” he said, watching me fiddle with the
dials. “You miss the shot because you’re too busy making it perfect”

  “You get to shape the shot,” I argued, “you decide how it comes out.”

  “Or you could just take it,” he said, raising his mirrorless and snapping a
portrait of me before I could react.

  I tried to take a portrait of him in return, but portraits aren’t really my thing.
I find birds are more interesting subjects, they don’t argue as much. The portrait
ended up being mediocre, poorly exposed, no depth of field.

  I still have that photo. I found it yesterday, tucked deep in a drawer of
keepsakes. I never realized how good it was. He’s mid-sentence, half-annoyed,
half-smiling, caught in the space between words. The kind of moment I would’ve
missed if I was messing with the aperture for the perfect, creamy bokeh.

  I went back to the flagship mirrorless that I’ve been eying up. I picked it up
again, feeling the weight of the shutter. It’s what I would’ve chosen before. It’s what
I thought made me a photographer.

  But now, I think about all the things he saw and I didn’t. How he captured
the in-between and how I waited for the perfect shot that often never arrived. And
how, in the end, I had expensive gear, but he was the one left with photos.

  He used to roll his eyes at things like that. “You like things that make your
life difficult,” he said, scrolling through the shots I’ve taken—too dark, too grainy, a
fraction of a second too late. “You like the idea of struggling with something, as if it
means more that way”

  Maybe he was right.

  Maybe that’s why I stayed after he was already half-gone, his mind already
in that city.

  I wonder if he still takes photos in the new city. If he still takes his photo
without obsessing over noise. If he still stops in the middle of the street to capture
something before it disappears. If he still doesn’t care if the camera in his hand is
the best or worst.

  If he still thinks of me when he sees a camera, the same way I do.

  I lifted the viewfinder to my eyes and hit the shutter button. There’s no
memory card inside, nothing to keep what I captured.

  Some things aren’t meant to be kept, even if you want to. I set the camera
back down and approach the counter for a new SD card.

  The man behind the counter watches me, waiting. “Didn’t find what you were
looking for?”

  “Not today, just this” I shake my head as he rings up the box.
      `,
    },
    {
      id: "secondhand-promise",
      title: "Secondhand Promise",
      content: `

>I wrote this in honor of a friend I used to know, not much to elaborate here.

The sky was the kind of gray that had no emotion left in it. Neither stormy nor clear, just suspended, indifferent. 
I sat on a bench that looked as tired as I felt, slouched between rusted metal arms and peeling wood. It was the same 
bench where I used to wait for the bus every evening, back when I thought the world was waiting for me, not the other way around.

I pulled a crumpled pack of cigarettes from my coat pocket, shook one loose, 
and lit it with an old Bic lighter I’d meant to throw away a dozen times. 
The smoke coiled upward like some quiet signal to no one.

That’s when I heard the footsteps. Quick, familiar, but not from anyone I could name. Then came the voice, smooth and unscarred.

“Haven’t quit yet?”

I turned my head slowly, already feeling the echo before I saw him.

He was me.

Not metaphorically. Not a hallucination, I don’t think. He looked exactly like I did at seventeen. His face was unmarked by time, by the years I had carried like a weight. He didn’t look tired.

I didn’t say anything right away. I just stared, cigarette burning between my fingers, smoke curling into the space between us.

“Well?” he asked.

“No,” I finally said. “I haven’t.”

He folded his arms. “We promised we’d quit before college.”

“We promised a lot of things,” I muttered.

He looked down at the ground. Kicked at a pebble. Then looked up at me again, more searching this time. Not the confrontational look of a teenager, but something deeper. The beginning of disillusionment.

“What happened?” he asked.

It wasn’t a bitter question. It was soft. Curious. Maybe even frightened.

I took another drag, letting the silence settle before I answered.

“Life,” I said. “The real kind. The kind you don't plan for.”

He raised an eyebrow. “That’s it?”

“No,” I said, “but it’s the start. Then came the jobs that went nowhere. The nights I didn’t sleep. The friends I lost to distance or time. The girl I thought I’d marry. The dreams that didn't survive.”

He sat down next to me. Close, but not too close.

“I still believe we’ll be something,” he said quietly.

I didn’t respond right away. I watched a bird land on the streetlight across from us, then fly away again as if it had somewhere better to be. Always somewhere better.

“You should,” I finally said.

“You don’t?”

I shrugged. “Harder to, now.”

We sat in silence for a minute. The city moved around us: cars humming, people talking, a siren wailing somewhere far off. But in our little pocket of the world, time had folded in on itself.

He looked at the cigarette again. “You said we’d only try it once.”

“I said we’d only try a lot of things once.”

“Do you still write?”

The question struck deeper than I expected.

“I try.”

“That’s not what I asked.”

I looked at him. At myself. “No,” I admitted. “Not really.”

He leaned forward, elbows on his knees. “You were going to write books. Travel. Speak three languages. Change something.”

I smiled, bitter. “Yeah. I was also going to quit smoking.”

“I don’t want to become you.”

That one stung. It wasn’t cruel, it was honest.

“I don’t want to be me either,” I said.

He looked up. His eyes were brimming with something I hadn't felt in a long time: belief. Not in the world, but in me. Or in who I was supposed to become.

“You still can change,” he said.

I looked away. “Maybe. But every time I’ve tried, I’ve failed. That stuff you believe in? It’s fragile.”

“So protect it,” he said.

I laughed once, sharp and hollow. “You think it's that easy?”

“I think,” he said, standing now, “that if you still carry the ashes, maybe there's still some fire in there.”

He looked at me for a long time. Not with hate. Not even with pity. Just with possibility.

“I really do believe we’ll be something,” he whispered.

“Then hold onto that,” I said, flicking the cigarette away.

He nodded once. No goodbyes, no dramatic exit. He just turned and walked off into the fog, toward the future I had once wanted, maybe still did.

I sat there a while longer. The cigarette had burned low, the filter hot between my fingers. I flicked it away. Watched it tumble into a puddle and hiss out with a quiet, unsatisfying end.

My hand reached toward my pocket.

But I stopped.

Maybe I had one more promise left in me.
      `,
    },
  ],
  poetry: [
    {
      id: "lighter",
      title: "Lighter",
      content: `
I find myself in the same place \
Because you, it remains the same place \
The clocks have all but frozen 

Fragile and flickering \
And it's the same night \
But the shading is different

My heart still weighs the same \
But I carry it lighter \
In this world turned upside down \
The feeling of letting go \
Takes some weight away

---

> I think this is one of my most poetic piece. Took me quite long to write actually, wrote the first draft at the end of g10 and finished it in g11. The first draft is this

"My heart still weighs the same \
but I carry it lighter \
In this world turned upside down \
Your head that rest upon my chest \
Takes some weight away" \

> actually fire, haven't got the chance to dedicate it to anyone though
      `,
    },
    {
      id: "i-took-you-out-of-my-wallet",
      title: "I took you out of my wallet",
      content: `
I took you out of my wallet \
Tucked it deep in a drawer \
Slid you into a box of old things

I took you out of my wallet \
As if letting go could feel like release \
As though distance could unlace the ties 

I took you out of my wallet \
But every time two lovers kiss \
I see a shadow of us lingering 

I took you out of my wallet \
But not from where you reside \
where you still hide 

I took you out of my wallet \
because my hands ache for a new weight \
And I lean toward the open door 

I took you out of my wallet \
So I leave us where the past belongs, \
in the quiet between two closing doors

> taking you out of my wallet was tough, in retrospect it definitely took me too long
      `,
    },
    {
      id: "last-one",
      title: "Last One",
      content: `
This is the last one \
I promise I'll quit \
It's expensive \
and it really stinks

and when I do \
I pledge to me \
and to you \
that I won't go back

This is the last one \
I swear it's true \
It won't be like last time \
I swear I'll follow through

> I wrote this from the perspective of my grandpa. Every family gathering he would promise us that he would quit, of course you can infer if he actually does or not. 
      `,
    },
     {
      id: "strangers",
      title: "Strangers",
      content: `
> This one is dedicated to... you! My (not) lovely friends.

We trade the days of innocence \
For late-night talks and consequence \
The world spins faster as we try \
To carve our names into the sky

Between the classes, streets, and screens \
We build our worlds on shifting dreams \
Will all these moments slip away \
With drifting ties in coming years  

But I’ll be here, and you’ll be alright \
No matter where our paths goes \
I hope, through all the fear \
That growing up won’t make us strangers.
      `,
    },
  ],
};
